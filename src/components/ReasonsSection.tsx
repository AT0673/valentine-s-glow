...
export const ReasonsSection = () => {
  ...
  const fetchReasons = async () => {
    const {
      data,
      error
    } = await supabase.from("reasons").select("*").order("display_order", {
      ascending: true
    });
    if (!error && data) {
      setReasons(data);
  ...
  if (isLoading) {
    return <section className="py-16 px-4 bg-gradient-soft">
  ...
  return <section className="py-16 px-4 bg-gradient-soft">
      <div className="max-w-4xl mx-auto">
        <h2 className="text-3xl md:text-4xl font-romantic text-center mb-4">Reasons I ADORE You</h2>
        <p className="text-center text-muted-foreground mb-12">
          Every single one of them, and so many more...
        </p>

        <div className="grid gap-6 md:grid-cols-2">
          {reasons.map((reason, index) => <Card key={reason.id} data-reason-card data-index={index} className={`bg-card/80 backdrop-blur-sm border-primary/20 shadow-soft hover:shadow-romantic transition-all duration-700 hover:scale-[1.02] ${visibleCards.has(index) ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{
